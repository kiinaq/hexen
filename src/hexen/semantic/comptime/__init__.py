...
class ComptimeAnalyzer:
    ...
    def can_comptime_types_mix_safely(self, left_type: HexenType, right_type: HexenType, target_type: Optional[HexenType]) -> bool:
        ...
        return self.type_ops.can_comptime_types_mix_safely(left_type, right_type, target_type)
    ...
    def handle_mixed_type_binary_operation(self, left_type, right_type, operator, target_type, error_callback, node) -> Optional[HexenType]:
        """Handle mixed type binary operations with BINARY_OPS.md patterns."""
        return self.binary_ops.handle_mixed_type_binary_operation(left_type, right_type, operator, target_type, error_callback, node)
    
    def resolve_arithmetic_operation_type(self, operator, left_type, right_type, target_type) -> Optional[HexenType]:
        """Resolve comptime-specific arithmetic operation types."""
        return self.binary_ops.resolve_arithmetic_operation_type(operator, left_type, right_type, target_type)
    
    def resolve_context_guided_arithmetic(self, left_type, right_type, target_type) -> Optional[HexenType]:
        """Resolve arithmetic operation with context guidance for comptime types."""
        return self.binary_ops.resolve_context_guided_arithmetic(left_type, right_type, target_type)
    
    def handle_mixed_type_comparison(self, operator, left_type, right_type, error_callback, node) -> Optional[HexenType]:
        """Handle mixed type comparison operations following BINARY_OPS.md rules."""
        return self.binary_ops.handle_mixed_type_comparison(operator, left_type, right_type, error_callback, node)
    
    def resolve_comptime_binary_operation(self, left_type, right_type, operator) -> HexenType:
        """Resolve result type for comptime binary operations."""
        return self.binary_ops.resolve_comptime_binary_operation(left_type, right_type, operator)
    ...
    def validate_variable_declaration_type_compatibility(self, value_type, var_type, value_node, variable_name, error_callback, node) -> bool:
        """Validate type compatibility for variable declarations with comptime awareness."""
        return self.declarations.validate_variable_declaration_type_compatibility(value_type, var_type, value_node, variable_name, error_callback, node)
    
    def should_preserve_comptime_for_declaration(self, mutability, inferred_type) -> HexenType:
        """Determine final variable type considering comptime type preservation rules."""
        return self.declarations.should_preserve_comptime_for_declaration(mutability, inferred_type)
    
    def analyze_declaration_type_inference_error(self, value_node, inferred_type, variable_name, error_callback, node) -> bool:
        """Analyze type inference errors with comptime awareness."""
        return self.declarations.analyze_declaration_type_inference_error(value_node, inferred_type, variable_name, error_callback, node)
    
    def should_preserve_comptime_type_in_declaration(self, mutability, inferred_type) -> bool:
        """Determine if comptime type should be preserved in variable declaration."""
        return self.declarations.should_preserve_comptime_type_in_declaration(mutability, inferred_type)
    ...
    def validate_comptime_literal_coercion(self, value, from_type, to_type, source_text=None) -> None:
        """Validate comptime literal can be safely coerced to target type."""
        return self.literals.validate_comptime_literal_coercion(value, from_type, to_type, source_text)
    ...
    def validate_assignment_comptime_literal(self, value_node, value_type, target_type) -> Optional[str]:
        """Validate comptime literal coercion in assignment context."""
        return self.literals.validate_assignment_comptime_literal(value_node, value_type, target_type)
    ...
    def resolve_conditional_comptime_types(self, branch_types, target_type) -> HexenType:
        """Resolve comptime types across conditional branches."""
        return self.type_ops.resolve_conditional_comptime_types(branch_types, target_type)
    
    def analyze_comptime_operands_in_binary_op(self, left_type, right_type, target_type) -> bool:
    ...
    def preserve_comptime_type_in_unary_op(self, operand_type, operator) -> HexenType:
        ...
        return self.binary_ops.preserve_comptime_type_in_unary_op(operand_type, operator)
    
    def validate_conditional_branch_compatibility(self, branch_types, target_type, error_callback, node) -> Optional[HexenType]:
        """Validate type compatibility across conditional branches and return unified type."""
        return self.block_eval.validate_conditional_branch_compatibility(branch_types, target_type, error_callback, node)
    
    def analyze_conditional_branch_with_target_context(self, branch_node, target_type, analyze_expression_callback) -> Optional[HexenType]:
        """Analyze conditional branch with target type context for assign statements."""
        return self.block_eval.analyze_conditional_branch_with_target_context(branch_node, target_type, analyze_expression_callback)
    ...
    def can_safely_adapt_comptime_types(self, left_type, right_type, target_type) -> bool:
        """Check if comptime types in a binary operation can safely adapt to the target type."""
        return self.binary_ops.can_safely_adapt_comptime_types(left_type, right_type, target_type)
    
    def should_skip_precision_loss_check_for_mixed_concrete(self, left_type, right_type, target_type) -> bool:
        """Check if precision loss check should be skipped for mixed concrete types."""
        return self.binary_ops.should_skip_precision_loss_check_for_mixed_concrete(left_type, right_type, target_type)
