...
class DeclarationSupport:
    ...
    def should_preserve_comptime_type_in_declaration(
        self, mutability: Mutability, inferred_type: HexenType
    ) -> bool:
        """
        ...
        if mutability == Mutability.IMMUTABLE and self.type_ops.is_comptime_type(
            inferred_type
        ):
            return True
    ...
    def validate_variable_declaration_type_compatibility(
        ...
        if is_precision_loss_operation(value_type, var_type):
            self._generate_declaration_precision_loss_error(
                value_type, var_type, error_callback, node
            )
            return False
        ...
        if not can_coerce(value_type, var_type):
            ...
            var_type_str = (
                var_type.value if hasattr(var_type, "value") else str(var_type)
            )
            value_type_str = (
                value_type.value if hasattr(value_type, "value") else str(value_type)
            )
    ...
    def analyze_declaration_type_inference_error(
        ...
        if inferred_type == HexenType.UNKNOWN:
            ...
            if node_type in ["binary_operation", "array_literal", "array_access"]:
                ...
                error_callback(
                    f"Cannot infer type for variable '{variable_name}'", node
                )
                return True
    ...
    def _generate_declaration_precision_loss_error(
        self, from_type: HexenType, to_type: HexenType, error_callback, node: Dict
    ) -> None:
    ...
    def _validate_comptime_literal_coercion(
        self, value, from_type, to_type, source_text
    ):
        """
