...
class BinaryOperations:
    ...
    def handle_mixed_type_binary_operation(
        ...
        has_comptime = self.type_ops.is_comptime_type(
            left_type
        ) or self.type_ops.is_comptime_type(right_type)
        ...
        if has_comptime and target_type is not None:
            ...
            if self.type_ops.is_comptime_type(
                left_type
            ) and self.type_ops.is_comptime_type(right_type):
    ...
    def handle_mixed_type_comparison(
        ...
        if is_numeric_type(left_type) and is_numeric_type(right_type):
            ...
            both_comptime = self.type_ops.is_comptime_type(
                left_type
            ) and self.type_ops.is_comptime_type(right_type)
    ...
    def resolve_arithmetic_operation_type(
        ...
        if target_type and (
            self.type_ops.is_comptime_type(left_type)
            or self.type_ops.is_comptime_type(right_type)
        ):
    ...
    def resolve_context_guided_arithmetic(
        self, left_type: HexenType, right_type: HexenType, target_type: HexenType
    ) -> Optional[HexenType]:
        """
    ...
    def resolve_comptime_binary_operation(
        self, left_type: HexenType, right_type: HexenType, operator: str
    ) -> HexenType:
        """
    ...
    def preserve_comptime_type_in_unary_op(
        self, operand_type: HexenType, operator: str
    ) -> HexenType:
        """
    ...
    def can_safely_adapt_comptime_types(
        ...
        has_comptime_operand = self.type_ops.is_comptime_type(
            left_type
        ) or self.type_ops.is_comptime_type(right_type)
        ...
        left_can_coerce = self.type_ops.is_comptime_type(left_type) and can_coerce(
            left_type, target_type
        )
        right_can_coerce = self.type_ops.is_comptime_type(right_type) and can_coerce(
    ...
    def should_skip_precision_loss_check_for_mixed_concrete(
        ...
        return can_coerce(left_type, target_type) and can_coerce(
            right_type, target_type
        )
    ...
    def analyze_assignment_comptime_operands(
        ...
        if not (
            ...
            return self.should_skip_precision_loss_check_for_mixed_concrete(
